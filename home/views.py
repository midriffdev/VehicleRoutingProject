...
def post_reports(request):
    if request.method == 'POST':
        ...
        wids = request.POST.get('wids')
        orderstatus = request.POST.get('selectedValueo')
        strat_date = request.POST.get('start_date')
        print(strat_date,"start ........")
        end_date = request.POST.get('end_date')
        ...
        try:
            ...
            if wids == 'All_Warehouse':
                ...
                if strat_date or end_date:
                    print(strat_date,"start 00000000")

                    try:
                        if strat_date:strat_date = datetime.strptime(strat_date, "%d %b, %Y").date()
                    except:
                        if strat_date:strat_date = datetime.strptime(strat_date, "%b. %d, %Y").date()
                        
                    try:
                        if end_date:end_date = datetime.strptime(end_date, "%d %b, %Y").date()
                    except:
                        if end_date:end_date = datetime.strptime(end_date, "%b. %d, %Y").date()

                    print(strat_date,"start date11111")
                    ...
                    if strat_date or end_date:
                        if strat_date and end_date:
                            ...
                            for i in  trucks_list:
                                i.total_deliveries=Order.objects.filter(assigned_truck=i,created_at__date__range=[strat_date, end_date]).count()
                ...
                print(strat_date,"strat_datestrat_datestrat_date")
...
def upload_orders(request):
    ...
    if request.method == 'POST':
        ...
        orders=Order.objects.filter().order_by('-order_status')[0:250]
        report_orders=Order.objects.filter(report_status=True)
